...
class PedidoController {
    ...
    static async confirmOrder(req, res) {
        ...
        //   total: number
        // }
        const { clienteId, estabelecimentoId, produtos, formaPagamento, total: totalCliente, paymentId, paymentStatus, paymentMethod, formaPagamentoEntrega, precisaTroco, trocoParaQuanto, enderecoEntrega } = req.body;
    ...
    static async create(req, res) {
        try {
            const { clienteId, estabelecimentoId, produtos, formaPagamento, total, paymentId, paymentStatus, paymentMethod, formaPagamentoEntrega, precisaTroco, trocoParaQuanto, enderecoEntrega } = req.body;
            // Debug logs
    ...
    static async cancelOrderWithRefund(req, res) {
        try {
            const { id } = req.params;
            ...
            if (!pedido.paymentId || pedido.paymentStatus !== 'approved') {
                ...
                notification_service_1.NotificationService.notifyPedidoStatusChange(pedido.id, pedido.status, 'cancelado');
            ...
            try {
                ...
                const canRefund = await mercadopago_service_1.MercadoPagoService.canRefundPayment(pedido.paymentId);
                ...
                if (pedido.paymentMethod === 'pix') {
                    ...
                    refund = await mercadopago_service_1.MercadoPagoService.createPixRefund(pedido.paymentId);
                }
                else {
                    console.log('🔄 Processando reembolso cartão...');
                    refund = await mercadopago_service_1.MercadoPagoService.createFullRefund(pedido.paymentId);
                }
    ...
    static async updatePaymentInfo(pedidoId, paymentData) {
        try {

...
class MercadoPagoService {
    ...
    static async createCustomer(email) {
        try {
            const response = await axios_1.default.post('https://api.mercadopago.com/v1/customers', { email }, {
                headers: {
    ...
    static async addCardToCustomer(customerId, token, paymentMethodId) {
        try {
            const response = await axios_1.default.post(`https://api.mercadopago.com/v1/customers/${customerId}/cards`, {
                token,
                payment_method_id: paymentMethodId
            }, {
                headers: {
    ...
    static async createPaymentWithSavedCard(paymentData) {
        try {
            ...
            try {
                const customerResponse = await axios_1.default.get(`https://api.mercadopago.com/v1/customers/${paymentData.customerId}`, {
                    headers: {
                        'Authorization': `Bearer ${process.env.MERCADO_PAGO_ACCESS_TOKEN}`,
                    },
                });
            ...
            try {
                const response = await axios_1.default.post('https://api.mercadopago.com/v1/payments', payment, {
                    headers: {
                        'Authorization': `Bearer ${process.env.MERCADO_PAGO_ACCESS_TOKEN}`,
                        'Content-Type': 'application/json',
                ...
                if (paymentError.response?.status === 500 && paymentError.response?.data?.error === 'internal_server_error') {
                    ...
                    const testTokenResponse = await axios_1.default.post('https://api.mercadopago.com/v1/card_tokens', {
                    ...
                    const fallbackResponse = await axios_1.default.post('https://api.mercadopago.com/v1/payments', fallbackPayment, {
                        headers: {
                            'Authorization': `Bearer ${process.env.MERCADO_PAGO_ACCESS_TOKEN}`,
                            'Content-Type': 'application/json',
                            'X-Idempotency-Key': `${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
